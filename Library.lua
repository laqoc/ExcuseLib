local InputService = game:GetService('UserInputService') -- metka
...
local Library = {
    Registry = {},
    RegistryMap = {},
    ActiveRegistry = {}, -- Добавлено для отслеживания видимых элементов
    HudRegistry = {},
...
local RainbowUpdateInterval = 1 / 30 -- Обновление 30 раз в секунду

table.insert(Library.Signals, RunService.Heartbeat:Connect(function(Delta)
    LastRainbowUpdate = LastRainbowUpdate + Delta
    if LastRainbowUpdate < RainbowUpdateInterval then return end
    LastRainbowUpdate = 0
...
function Library:AddToRegistry(Instance, Properties, IsHud)
    ...
    Instance:GetPropertyChangedSignal('Visible'):Connect(function()
        if Instance.Visible then
            Library.ActiveRegistry[Instance] = Data
        else
            Library.ActiveRegistry[Instance] = nil
        end
    end)
...
function Library:UpdateColorsUsingRegistry()
    for Instance, Data in next, Library.ActiveRegistry do
...
do
    ...
    function Funcs:AddColorPicker(Idx, Info)
        ...
        Library:GiveSignal(InputService.InputBegan:Connect(function(Input)
            ...
            elseif Input.UserInputType == Enum.UserInputType.MouseButton2 and ContextMenu.Container.Visible then
...
do
            ...
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame() then
                ...
                Connection = InputService.InputChanged:Connect(function(Input)
                    if Input.UserInputType == Enum.UserInputType.MouseMovement then
                        ...
                        local nX = math.clamp(gPos + (mPos - mPos) + Diff, 0, Slider.MaxSize)
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        local Dropdown = {
            ...
            Callback = Info.Callback or function(Value) end,
            Buttons = {} -- Добавлено для переиспользования кнопок
        }
        ...
        function Dropdown:BuildDropdownList()
            local Values = Dropdown.Values
            local ExistingButtons = Dropdown.Buttons
            local Count = 0

            for Idx, Value in next, Values do
                Count = Count + 1
                local Table = ExistingButtons[Value] or {}

                local Button
                if Table.Button then
                    Button = Table.Button
                    Table.ButtonLabel.Text = Value
                else
                    Button = Library:Create('Frame', {
                        BackgroundColor3 = Library.MainColor,
                        BorderColor3 = Library.OutlineColor,
                    ...
                    Table.ButtonLabel = Library:CreateLabel({
                        Active = false,
                ...
                function Table:UpdateButton()
                    ...
                    Table.ButtonLabel.TextColor3 = Selected and Library.AccentColor or Library.FontColor
                    Library.RegistryMap[Table.ButtonLabel].Properties.TextColor3 = Selected and 'AccentColor' or 'FontColor'
                end

                Table.Button.InputBegan:Connect(function(Input)
                    if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                        ...
                        if Info.Multi then
                            ...
                            for _, OtherTable in next, Dropdown.Buttons do
            ...
            -- Очистка неиспользуемых кнопок
            for Value, Table in next, Dropdown.Buttons do
